...
class ValidationError(Exception):
    def __init__(self, gate, code, message, *, run_dir=None, path=None, node_id=None):
...
def fail(gate, code, message, *, run_dir=None, path=None, node_id=None):
    raise ValidationError(gate, code, message, run_dir=run_dir, path=path, node_id=node_id)
...
def read_markdown_contract(path, gate, code, *, run_dir=None, node_id=None):
    try:
        payload, body = split_markdown_json(read_text(path))
    except FileNotFoundError:
        fail(gate, code, f"Missing file: {path}", run_dir=run_dir, path=path, node_id=node_id)
    ...
    if payload is None:
        fail(gate, code, f"Missing first fenced JSON block in {path}", run_dir=run_dir, path=path, node_id=node_id)
    return payload, body
...
def require_keys(obj, required, gate, code, *, run_dir, path, node_id=""):
...
def require_non_empty_string(value, field_name, gate, code, *, run_dir, path, node_id=""):
...
def require_enum(value, field_name, allowed, gate, code, *, run_dir, path, node_id=""):
...
def require_list(value, field_name, gate, code, *, run_dir, path, node_id=""):
...
def require_non_empty_string_list(value, field_name, gate, code, *, run_dir, path, node_id=""):
    require_list(value, field_name, gate, code, run_dir=run_dir, path=path, node_id=node_id)
    if not value:
        fail(
            gate,
            code,
    ...
    for index, item in enumerate(value, start=1):
        require_non_empty_string(item, f"{field_name}[{index}]", gate, code, run_dir=run_dir, path=path, node_id=node_id)
...
def validate_tree_document(tree, tree_path):
    run_dir = tree_path.parent
    require_keys(tree, ["topic", "slug", "created", "nodes"], "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path)
    require_non_empty_string(tree["topic"], "topic", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path)
    require_non_empty_string(tree["slug"], "slug", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path)
    require_non_empty_string(tree["created"], "created", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path)
    load_valid_framing_brief(run_dir, expected_topic=tree["topic"])
    require_list(tree["nodes"], "nodes", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path)
    if not tree["nodes"]:
        fail("TREE_GATE", "TREE_INCOMPLETE", "tree.md must contain at least one node", run_dir=run_dir, path=tree_path)
    ...
    for node in tree["nodes"]:
        ...
        require_non_empty_string(node_id, "node_id", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path)
        if node_id in node_ids:
            fail("TREE_GATE", "TREE_INCOMPLETE", f"Duplicate node_id '{node_id}' in tree.md", run_dir=run_dir, path=tree_path, node_id=node_id)
        node_ids.add(node_id)
        require_non_empty_string(node["title"], "title", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path, node_id=node_id)
        require_non_empty_string(node["question"], "question", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path, node_id=node_id)
        require_enum(node["source_tier"], "source_tier", SOURCE_TIERS, "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path, node_id=node_id)
        require_enum(node["priority"], "priority", NODE_PRIORITIES, "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path, node_id=node_id)
        require_enum(node["status"], "status", NODE_STATUSES, "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path, node_id=node_id)
        require_list(node["prerequisites"], "prerequisites", "TREE_GATE", "TREE_INCOMPLETE", run_dir=run_dir, path=tree_path, node_id=node_id)
        if not isinstance(node["owner"], str):
            fail("TREE_GATE", "TREE_INCOMPLETE", f"Field 'owner' must be a string for node '{node_id}'", run_dir=run_dir, path=tree_path, node_id=node_id)
        if not isinstance(node["skip_reason"], str):
            fail("TREE_GATE", "TREE_INCOMPLETE", f"Field 'skip_reason' must be a string for node '{node_id}'", run_dir=run_dir, path=tree_path, node_id=node_id)
    ...
    if topo_sort(tree["nodes"]) is None:
        fail("TREE_GATE", "PREREQUISITE_CYCLE", "Tree prerequisites contain a cycle", run_dir=run_dir, path=tree_path)


def validate_dispatch_document(dispatch, dispatch_path, tree):
    run_dir = dispatch_path.parent
    require_keys(dispatch, ["dispatch"], "DISPATCH_GATE", "DISPATCH_ORPHAN", run_dir=run_dir, path=dispatch_path)
    require_list(dispatch["dispatch"], "dispatch", "DISPATCH_GATE", "DISPATCH_ORPHAN", run_dir=run_dir, path=dispatch_path)
    ...
    for row in dispatch["dispatch"]:
        ...
        node_id = row["node_id"]
        if node_id in seen:
            fail("DISPATCH_GATE", "DISPATCH_ORPHAN", f"Duplicate dispatch row for '{node_id}'", run_dir=run_dir, path=dispatch_path, node_id=node_id)
        seen.add(node_id)
        require_non_empty_string(node_id, "node_id", "DISPATCH_GATE", "DISPATCH_ORPHAN", run_dir=run_dir, path=dispatch_path, node_id=node_id)
        require_non_empty_string(row["owner"], "owner", "DISPATCH_GATE", "DISPATCH_ORPHAN", run_dir=run_dir, path=dispatch_path, node_id=node_id)
        ...
        require_non_empty_string(row["artifact_path"], "artifact_path", "DISPATCH_GATE", "DISPATCH_ORPHAN", run_dir=run_dir, path=dispatch_path, node_id=node_id)
        if node_id not in tree_nodes:
            fail("DISPATCH_GATE", "DISPATCH_ORPHAN", f"Dispatch row '{node_id}' does not match a tree node", run_dir=run_dir, path=dispatch_path, node_id=node_id)
        if tree_nodes[node_id]["status"] == "skipped":
            fail("DISPATCH_GATE", "DISPATCH_ORPHAN", f"Skipped node '{node_id}' may not be dispatched", run_dir=run_dir, path=dispatch_path, node_id=node_id)
...
def validate_artifact_document(artifact, artifact_path, tree):
    ...
    require_non_empty_string(node_id, "node_id", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path)
    if artifact_path.stem != node_id:
        fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"Artifact filename must match node_id '{node_id}'", run_dir=run_dir, path=artifact_path, node_id=node_id)
    tree_nodes = {node["node_id"]: node for node in tree["nodes"]}
    if node_id not in tree_nodes:
        fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"Artifact node_id '{node_id}' is not present in tree.md", run_dir=run_dir, path=artifact_path, node_id=node_id)
    if tree_nodes[node_id]["status"] == "skipped":
        fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"Skipped node '{node_id}' must not produce an artifact", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_non_empty_string(artifact["title"], "title", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    if not isinstance(artifact["scope_confirmed"], bool):
        fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"scope_confirmed must be boolean for '{node_id}'", run_dir=run_dir, path=artifact_path, node_id=node_id)
    if not artifact["scope_confirmed"]:
        fail("ARTIFACT_GATE", "SCOPE_NOT_CONFIRMED", f"Artifact '{node_id}' did not confirm scope", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_non_empty_string(artifact["scope_evidence"], "scope_evidence", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_enum(artifact["confidence_level"], "confidence_level", CONFIDENCE_LEVELS, "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_list(artifact["prerequisite_nodes"], "prerequisite_nodes", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    ...
    require_list(artifact["sources"], "sources", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_list(artifact["claims"], "claims", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_list(artifact["evidence"], "evidence", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_non_empty_string_list(artifact["examples"], "examples", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_list(artifact["contradictions"], "contradictions", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_list(artifact["coverage_gaps"], "coverage_gaps", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    require_non_empty_string_list(artifact["teaching_hooks"], "teaching_hooks", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
    if not artifact["sources"]:
        fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"Artifact '{node_id}' must include at least one source", run_dir=run_dir, path=artifact_path, node_id=node_id)
    if not artifact["claims"]:
        fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"Artifact '{node_id}' must include at least one claim", run_dir=run_dir, path=artifact_path, node_id=node_id)
    ...
    for source in artifact["sources"]:
        ...
        require_non_empty_string(source_id, "source_id", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        require_non_empty_string(source["source"], "source", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        require_non_empty_string(source["title"], "title", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        require_non_empty_string(source["excerpt"], "excerpt", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        require_enum(source["tier"], "tier", SOURCE_TIERS, "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        if source_id in source_ids or source_id in other_source_ids:
            fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"Duplicate source_id '{source_id}' in package", run_dir=run_dir, path=artifact_path, node_id=node_id)
        source_ids.add(source_id)

    evidence_ids = set()
    for evidence_id in artifact["evidence"]:
        require_non_empty_string(evidence_id, "evidence[]", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        if evidence_id not in source_ids:
            fail("ARTIFACT_GATE", "CLAIM_UNTRACEABLE", f"Evidence id '{evidence_id}' is missing from sources in '{node_id}'", run_dir=run_dir, path=artifact_path, node_id=node_id)
    ...
    for claim in artifact["claims"]:
        ...
        require_non_empty_string(claim_id, "claim_id", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        require_non_empty_string(claim["claim"], "claim", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        require_list(claim["source_ids"], "source_ids", "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        require_enum(claim["confidence"], "confidence", CONFIDENCE_LEVELS, "ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", run_dir=run_dir, path=artifact_path, node_id=node_id)
        if claim_id in claim_ids or claim_id in other_claim_ids:
            fail("ARTIFACT_GATE", "ARTIFACT_INCOMPLETE", f"Duplicate claim_id '{claim_id}' in package", run_dir=run_dir, path=artifact_path, node_id=node_id)
        if not claim["source_ids"]:
            fail("ARTIFACT_GATE", "CLAIM_UNTRACEABLE", f"Claim '{claim_id}' must reference at least one source", run_dir=run_dir, path=artifact_path, node_id=node_id)
...
def log_event(run_dir, event, *, gate="", status="ok", code="", path="", node_id="", message="", details=None):
    record = {
        "timestamp": now_iso(),
        "event": event,
        "gate": gate,
...
def write_failure_report(error):
    ...
    recovery = RECOVERY_HINTS.get(
        error.code,
        "Fix the reported issue, update the affected artifact or tree state, and rerun the gate.",
    )
    payload = {
        "gate": error.gate,
        "code": error.code,
...
def command_init_tree(args):
    ...
    if (run_dir / "tree.md").exists():
        fail("TOPIC_GATE", "TREE_INCOMPLETE", f"Run package already exists at {run_dir}", run_dir=run_dir, path=run_dir / "tree.md")
...
def command_validate_tree(args):
    ...
    print(f"TREE_GATE ok: {len(tree['nodes'])} nodes")
...
def command_validate_dispatch(args):
    ...
    print(f"DISPATCH_GATE ok: {len(dispatch['dispatch'])} rows")
...
def command_validate_artifact(args):
    ...
    print(f"ARTIFACT_GATE ok: {artifact['node_id']}")
...
def ensure_any_artifact_aligns_with_framing(run_dir, tree, framing, gate, path):
    framing_terms = framing_alignment_terms(framing)
    if not framing_terms:
        fail(gate, "SYNTHESIS_UNCOVERED", "framing_brief lacks checkable success_criteria or primary_confusion terms", run_dir=run_dir, path=path)
    for node in tree["nodes"]:
...
def command_validate_synthesis(args):
    ...
    if synthesis["framing_brief"] != framing:
        fail("SYNTHESIS_GATE", "SYNTHESIS_UNCOVERED", "synthesis.md dropped or changed framing_brief", run_dir=run_dir, path=run_dir / "synthesis.md")
    for field_name in ["learner_goal", "primary_confusion", "entry_point"]:
        if framing[field_name] not in synthesis_body:
            fail("SYNTHESIS_GATE", "SYNTHESIS_UNCOVERED", f"synthesis.md body is missing framing field '{field_name}'", run_dir=run_dir, path=run_dir / "synthesis.md")
    ...
    if covered != expected_covered:
        fail("SYNTHESIS_GATE", "SYNTHESIS_UNCOVERED", "synthesis.md does not cover every non-skipped node", run_dir=run_dir, path=run_dir / "synthesis.md")
    if skipped != expected_skipped:
        fail("SYNTHESIS_GATE", "SYNTHESIS_UNCOVERED", "synthesis.md does not list every skipped node", run_dir=run_dir, path=run_dir / "synthesis.md")
    skipped_nodes = [node for node in tree["nodes"] if node["status"] == "skipped"]
    if not ensure_skipped_nodes_in_body(synthesis_body, skipped_nodes):
        fail("SYNTHESIS_GATE", "SYNTHESIS_UNCOVERED", "Skipped nodes or reasons are missing from synthesis body", run_dir=run_dir, path=run_dir / "synthesis.md")
    ...
    for claim_id in synthesis["claim_ids"]:
        if claim_id not in claim_rows:
            fail("SYNTHESIS_GATE", "CLAIM_UNTRACEABLE", f"Claim '{claim_id}' is missing from claim_ledger.md", run_dir=run_dir, path=run_dir / "synthesis.md")
        for source_id in claim_rows[claim_id]["source_ids"]:
            if source_id not in source_rows:
                fail("SYNTHESIS_GATE", "CLAIM_UNTRACEABLE", f"Claim '{claim_id}' references missing source '{source_id}'", run_dir=run_dir, path=run_dir / "synthesis.md")
    ...
    if not expected_gaps.issubset(set(synthesis["open_gaps"])):
        fail("SYNTHESIS_GATE", "GAP_DROPPED", "synthesis.md dropped one or more coverage gaps", run_dir=run_dir, path=run_dir / "synthesis.md")
    if not expected_contradictions.issubset(set(synthesis["open_contradictions"])):
        fail("SYNTHESIS_GATE", "GAP_DROPPED", "synthesis.md dropped one or more contradictions", run_dir=run_dir, path=run_dir / "synthesis.md")
    ensure_any_artifact_aligns_with_framing(run_dir, tree, framing, "SYNTHESIS_GATE", run_dir / "synthesis.md")

    print(f"SYNTHESIS_GATE ok: {len(covered)} covered, {len(skipped)} skipped")
...
def build_parser():
    parser = argparse.ArgumentParser(description="Deep topic gate workflow helper for master-zhuyan.")
...
def main(argv=None):
    ...
    try:
        ...
        print(f"{error.gate} {error.code}: {error.message}", file=sys.stderr)
        return 1
    return 0
