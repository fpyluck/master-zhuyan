...
SCRIPT = Path(__file__).resolve().parent.parent / "scripts" / "learning_artifacts.py"
...
class LearningArtifactsCliTests(unittest.TestCase):
    ...
    def test_positive_card_flow(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("CARD_GATE ok", result.stdout)
    ...
    def test_positive_map_flow(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("MAP_GATE ok", result.stdout)
    ...
    def test_positive_report_flow(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("REPORT_GATE ok", result.stdout)
