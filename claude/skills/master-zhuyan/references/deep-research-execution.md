...
Read all supplied sources. For each source, build a source map entry (see `references/filesystem-execution.md` section 2). Then extract evidence claims into the Evidence Ledger.
...
Record `notes/chapter_plan.md` as in the normal longform workflow (`references/filesystem-execution.md` section 6).
