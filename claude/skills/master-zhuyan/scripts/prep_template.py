...
加载条件：当前请求中出现上方 `aliases` 中任一短语。“{name}”作为话题裸词仍按当前主题、来源或普通措辞处理。

## 备课注意力

1. 把 prompt 当作软性注意力（角色定位、资料优先级、学习目标、解释风格、安全边界），不要变成死板分支或必填表格。
2. 当前请求和本轮材料优先于本模板；与 longform-first 冲突时保留 longform 容器，只调整章节内写法。
3. 涉及医学、法律、金融、工程安全等高风险内容时，继续保留来源核对和不确定性标注。
