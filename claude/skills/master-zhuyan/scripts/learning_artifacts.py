...
class ValidationError(Exception):
    def __init__(self, gate, code, message, *, path=None):
...
def fail(gate, code, message, *, path=None):
    raise ValidationError(gate, code, message, path=path)
...
def read_markdown_contract(path, gate, code):
    try:
        payload, body = split_markdown_json(read_text(path))
    except FileNotFoundError:
        fail(gate, code, f"Missing file: {path}", path=path)
    except json.JSONDecodeError as exc:
        fail(gate, code, f"Invalid JSON block in {path}: {exc}", path=path)
    if payload is None:
        fail(gate, code, f"Missing first fenced JSON block in {path}", path=path)
    return payload, body
...
def require_keys(obj, required, gate, code, *, path):
    missing = [key for key in required if key not in obj]
    if missing:
        fail(gate, code, f"Missing required fields {missing} in {path}", path=path)


def require_non_empty_string(value, field_name, gate, code, *, path):
    if not isinstance(value, str) or not value.strip():
        fail(gate, code, f"Field '{field_name}' must be a non-empty string in {path}", path=path)


def require_enum(value, field_name, allowed, gate, code, *, path):
    if value not in allowed:
        fail(gate, code, f"Field '{field_name}' must be one of {sorted(allowed)} in {path}", path=path)


def require_list(value, field_name, gate, code, *, path):
    if not isinstance(value, list):
        fail(gate, code, f"Field '{field_name}' must be a list in {path}", path=path)


def validate_card_document(card, card_path):
    path = Path(card_path)
    require_keys(card, CARD_FIELDS, "CARD_GATE", "CARD_INCOMPLETE", path=path)
    for field_name in ["topic", "slug", "created", "learner_goal", "core_model", "anchor_example"]:
        require_non_empty_string(card[field_name], field_name, "CARD_GATE", "CARD_INCOMPLETE", path=path)
    for field_name in ["key_boundaries", "misconceptions", "retrieval_hooks", "expansion_links"]:
        require_list(card[field_name], field_name, "CARD_GATE", "CARD_INCOMPLETE", path=path)
    if not card["retrieval_hooks"]:
        fail("CARD_GATE", "CARD_INCOMPLETE", f"Field 'retrieval_hooks' must be a non-empty list in {path}", path=path)
    require_enum(card["status"], "status", CARD_STATUS_VALUES, "CARD_GATE", "CARD_INCOMPLETE", path=path)
    provenance = card["provenance"]
    if not isinstance(provenance, dict):
        fail("CARD_GATE", "CARD_INCOMPLETE", f"Field 'provenance' must be an object in {path}", path=path)
    for prov_field in ["source_facts", "teacher_framing"]:
        if prov_field not in provenance:
            fail("CARD_GATE", "CARD_INCOMPLETE", f"provenance missing required field '{prov_field}' in {path}", path=path)
        require_list(provenance[prov_field], f"provenance.{prov_field}", "CARD_GATE", "CARD_INCOMPLETE", path=path)


def validate_map_document(map_data, map_path):
    path = Path(map_path)
    require_keys(map_data, MAP_FIELDS, "MAP_GATE", "MAP_INCOMPLETE", path=path)
    for field_name in ["topic", "slug", "created", "start_node"]:
        require_non_empty_string(map_data[field_name], field_name, "MAP_GATE", "MAP_INCOMPLETE", path=path)
    for field_name in ["nodes", "edges", "open_questions", "next_steps"]:
        require_list(map_data[field_name], field_name, "MAP_GATE", "MAP_INCOMPLETE", path=path)
    require_enum(map_data["status"], "status", MAP_STATUS_VALUES, "MAP_GATE", "MAP_INCOMPLETE", path=path)
    if not map_data["nodes"]:
        fail("MAP_GATE", "MAP_INCOMPLETE", f"Map must contain at least one node in {path}", path=path)
    node_ids = set()
    for node in map_data["nodes"]:
        require_keys(node, NODE_FIELDS, "MAP_GATE", "MAP_INCOMPLETE", path=path)
        node_id = node["node_id"]
        require_non_empty_string(node_id, "node_id", "MAP_GATE", "MAP_INCOMPLETE", path=path)
        if node_id in node_ids:
            fail("MAP_GATE", "MAP_INCOMPLETE", f"Duplicate node_id '{node_id}' in {path}", path=path)
        node_ids.add(node_id)
        require_non_empty_string(node["title"], "title", "MAP_GATE", "MAP_INCOMPLETE", path=path)
        require_non_empty_string(node["question"], "question", "MAP_GATE", "MAP_INCOMPLETE", path=path)
        require_non_empty_string(node["relation"], "relation", "MAP_GATE", "MAP_INCOMPLETE", path=path)
        require_enum(node["priority"], "priority", NODE_PRIORITIES, "MAP_GATE", "MAP_INCOMPLETE", path=path)
        require_enum(node["status"], "status", NODE_STATUSES, "MAP_GATE", "MAP_INCOMPLETE", path=path)
    if map_data["start_node"] not in node_ids:
        fail("MAP_GATE", "MAP_INCOMPLETE", f"start_node '{map_data['start_node']}' not found in nodes in {path}", path=path)
    for edge in map_data["edges"]:
        require_keys(edge, EDGE_FIELDS, "MAP_GATE", "MAP_INCOMPLETE", path=path)
        for endpoint in ["source", "target"]:
            if edge[endpoint] not in node_ids:
                fail("MAP_GATE", "MAP_INCOMPLETE", f"Edge {endpoint} '{edge[endpoint]}' is not a known node_id in {path}", path=path)
        require_non_empty_string(edge["relation"], "relation", "MAP_GATE", "MAP_INCOMPLETE", path=path)


def validate_report_document(report, report_path):
    path = Path(report_path)
    require_keys(report, REPORT_FIELDS, "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
    for field_name in ["topic", "slug", "created", "learner_goal", "core_model"]:
        require_non_empty_string(report[field_name], field_name, "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
    require_enum(report["explanatory_type"], "explanatory_type", EXPLANATORY_TYPES, "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
    for field_name in [
        "coverage_kernel",
        "reasoning_chain",
        ...
        require_list(report[field_name], field_name, "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
    if not report["coverage_kernel"]:
        fail("REPORT_GATE", "REPORT_INCOMPLETE", f"Field 'coverage_kernel' must be a non-empty list in {path}", path=path)
    if not report["reasoning_chain"]:
        fail("REPORT_GATE", "REPORT_INCOMPLETE", f"Field 'reasoning_chain' must be a non-empty list in {path}", path=path)
    if not report["load_bearing_branches"]:
        fail("REPORT_GATE", "REPORT_INCOMPLETE", f"Field 'load_bearing_branches' must be a non-empty list in {path}", path=path)
    if not report["retrieval_hooks"]:
        fail("REPORT_GATE", "REPORT_INCOMPLETE", f"Field 'retrieval_hooks' must be a non-empty list in {path}", path=path)
    require_enum(report["status"], "status", REPORT_STATUS_VALUES, "REPORT_GATE", "REPORT_INCOMPLETE", path=path)

    for index, step in enumerate(report["reasoning_chain"], start=1):
        require_keys(step, REASONING_STEP_FIELDS, "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
        require_non_empty_string(step["step"], f"reasoning_chain[{index}].step", "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
    ...
    for index, branch in enumerate(report["load_bearing_branches"], start=1):
        require_keys(branch, REPORT_BRANCH_FIELDS, "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
        ...
        title = branch["title"]
        require_non_empty_string(title, f"load_bearing_branches[{index}].title", "REPORT_GATE", "REPORT_INCOMPLETE", path=path)
        require_non_empty_string(
            branch["explanation"],
...
def command_validate_card(args):
    ...
    card, _ = read_markdown_contract(card_path, "CARD_GATE", "CARD_INCOMPLETE")
    validate_card_document(card, card_path)
    print(f"CARD_GATE ok: {card_path.name}")
...
def command_validate_map(args):
    ...
    map_data, _ = read_markdown_contract(map_path, "MAP_GATE", "MAP_INCOMPLETE")
    validate_map_document(map_data, map_path)
    print(f"MAP_GATE ok: {map_path.name} ({len(map_data['nodes'])} nodes)")
...
def command_validate_report(args):
    ...
    report, _ = read_markdown_contract(report_path, "REPORT_GATE", "REPORT_INCOMPLETE")
    validate_report_document(report, report_path)
    print(f"REPORT_GATE ok: {report_path.name} ({len(report['load_bearing_branches'])} branches)")
...
def render_report(report):
    ...
    for branch in report["load_bearing_branches"]:
        ...
        lines.append("")
        lines.append(f"Boundary or failure mode: {branch['boundary_or_failure_mode']}")
        lines.append("")
...
def build_parser():
    parser = argparse.ArgumentParser(description="Knowledge artifact helpers for master-zhuyan.")
    ...
    render_card_p.add_argument("--out", default=None)
    ...
    render_map_p.add_argument("--out", default=None)
    ...
    render_report_p.add_argument("--out", default=None)
